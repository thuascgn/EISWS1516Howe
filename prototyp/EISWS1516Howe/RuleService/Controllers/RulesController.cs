...
namespace RuleService.Controllers
{
    public class RulesController : ApiController
    {
        ...
        [HttpPost]
        [Route("api/rules")]
        public HttpResponseMessage Post([FromBody]RuleTask ruleTask)
        {
            long insertedId = -1;
            //ruleProvider = new RuleProvider();
            insertedId = ruleProvider.Create(ruleTask.Rule);

            if (insertedId > 0)
            {
                rule = ruleProvider.Read((int) insertedId);
                if (rule != null)
                {
                    response.StatusCode = HttpStatusCode.Created;
                    //ruleTask.Id = (int)insertedId;
                    response.Content = new StringContent(ruleTask.ToJson()); 
                }
            }
            else
            {
