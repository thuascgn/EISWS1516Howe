...
namespace TaskService.Providers
{
    public class TaskProvider
    {
        ...
        private String db = "task_service";
        ...
        public TaskProvider() {
            TaskConnection = "server=localhost;port=9001;user id=admin;pwd=rak20adm/n16;database=" + db + ";";
        }
        ...
        public long Create(RuleTask task)
        {
            ...
            SqlCmd = "INSERT INTO "+ db_table_task +" (id, priority, channel, isLocked, " + 
                ...
                "values (null," + task.Priority + ", '" + task.Department.ToLower() + "', " + task.IsLocked +
            ...
            try
            {
                ...
                if (reader.HasRows)
                {
                    while (reader.Read())
                    {
                        ...
                        task.Department = (string)reader[2];
                        ...
                        task.Rule.Attribution.ProjectNumber = (string)reader[14];
                    ...
                    task.IsLocked = true;
                    SqlCmd = "UPDATE " + db_table_task + " SET isLocked=" + task.IsLocked + " WHERE id=" + task.Id + ";";
                    cmd = new MySqlCommand(SqlCmd , conn);
        ...
        /// <summary>
        /// Updates the current Task from Database
        /// </summary>
        internal Boolean Update(RuleTask task)
        {
            int success = -1;
            SqlCmd = "UPDATE tbl_tasks SET priority=" + task.Priority + ", channel='" + task.Department.ToLower() + "', isLocked=" + task.IsLocked +
                ", document_Id=" + task.Document.Id + ", document_Guid='" + task.Document.DocumentGuid+ "', document_Path='" + task.Document.Path +
                "', document_Sender='" + task.Document.Sender + "', document_Number='" + task.Document.Number + "', document_AccountingText='" + task.Document.AccountingText + 
